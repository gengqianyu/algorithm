...
func (h *HTree) Sort() []int {
	...
	//		5		6
	//	  8	  9
	//第五次去构建大顶堆的时候 l=1 j也=1 不满足j<l的表达式 直接不用调整了
	for l := len(h.s) - 1; l > 0; l-- {
		//将大顶堆最大元素顶元素与末尾元素交换，将最大元素沉到切片的末尾
		//注意l是递减的，所以第二次是和倒数第二个元素进行交换
		h.s[l], h.s[0] = h.s[0], h.s[l]
...
func (h *HTree) heap(i, l int) {
	...
	//(i << 1) + 1 就是i的左子节点 随着调用l的值越来越小 构建大顶堆的深度也会越来越小
