...
// 静态链接。程序在运行前编译的最后进行模块链接。运行时直接分配内存空间就可以了。此种方式，程序执行效率高，但是单位时间内占用的内存高。
// 动态链接：程序在运行时动态链接，按需加载模块。运行是又需要链接，又需要分配内存空间。此种方式，单位时间内占用内存低，程序执行效率稍差一点。
// 			程序在装入时动态链接，程序在装入内存运行之前链接，又需要分配内存空间。此种方式，内存占用和静态链接差不多，但是程序执行效率稍差一点。
...
// 这些指令的编址都是逻辑地址。每一个模块编址都是从逻辑地址0开始的(因为重定向寄存器基址可变，所以最后的物理地址还是不一样的)。就这样编译形成了二进制机器语言。
...
//5,cpu读取物理内存地址(重定位基址(可变)+指令逻辑地址)，取址执行。
package main
