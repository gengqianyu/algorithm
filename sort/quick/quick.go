...
func Sort(s []int, l, r int) []int {
	//1,让a,b前后两个索引位置，开始从左右两边向中间移动
	//2,a负责找到一个比中轴值m小的值，将这a位置上的值放到中轴索引p的位置，把中轴索引p位置为a位置
	//3,b负责找到一个比中轴值m大的值，将这b位置上的值放到中轴索引p的位置(原来a的位置) ，把中轴索引p置为b位置
	//4,下一轮重复2,3步骤，找到一个比m小，比m大的值，通过p->a->b的方式，一直跟着a和b进行前后变化。
	//	通过将a和b位置上的值，赋值给p位置(s[p]=s[a] s[p]=s[b])，实现小数(a位置的值s[a])放左边，大数(b位置的值s[b])放右边。
